				...
				<div class="table table-responsive">
					<table class="table table-bordered table-striped datatable">
						...
						<tr>
						  ...
						  <td><a href="assets/upload/<?=$ls->pembayaran_bukti?>" target="_blank"><img src="assets/upload/<?=$ls->pembayaran_bukti?>" class="img-responsive img-thumbnail" width="100" height="100" ></a></td>
						  <td><?=$arrayst[$ls->pembayaran_status]?></td>
						  <td>
							<div class="btn-group">
								<?php if($ls->pembayaran_status=='0') { ?>
								<a href="<?=base_url()?>pembayaranadmin/validasi/<?=$ls->pembayaran_id?>/<?=$ls->pembayaran_tagihan_id?>" onclick="return confirm('Validasi Data Ini?')" class="btn btn-info btn-xs" >Validasi</a>
								<a href="<?=base_url()?>pembayaranadmin/tolak/<?=$ls->pembayaran_id?>/<?=$ls->pembayaran_tagihan_id?>" onclick="return confirm('Tolak Data Ini?')" class="btn btn-warning btn-xs" >Tolak</a>								
								<?php }?>	
								<a href="<?=base_url()?>pembayaranadmin/destroy/<?=$ls->pembayaran_id?>" onclick="return confirm('Hapus Data Ini?')" class="btn btn-danger btn-xs" >hapus</a>								
							</div>
						  </td>
						</tr>
						<?php }}?>
